...
data_nestor_b = pd.read_csv(fullpath,sep=';')
print(data_nestor_b.columns.values)
print(data_nestor_b.shape)
print(data_nestor_b.describe())
print(data_nestor_b.dtypes) 
print(data_nestor_b.head(5))
print(data_nestor_b.age.describe())
"""
...
"""
print(data_nestor_b['education'].unique())
import numpy as np
data_nestor_b['education']=np.where(data_nestor_b['education'] =='basic.9y', 'Basic', data_nestor_b['education'])
data_nestor_b['education']=np.where(data_nestor_b['education'] =='basic.6y', 'Basic', data_nestor_b['education'])
data_nestor_b['education']=np.where(data_nestor_b['education'] =='basic.4y', 'Basic', data_nestor_b['education'])
data_nestor_b['education']=np.where(data_nestor_b['education'] =='university.degree', 'University Degree', data_nestor_b['education'])
data_nestor_b['education']=np.where(data_nestor_b['education'] =='professional.course', 'Professional Course', data_nestor_b['education'])
data_nestor_b['education']=np.where(data_nestor_b['education'] =='high.school', 'High School', data_nestor_b['education'])
data_nestor_b['education']=np.where(data_nestor_b['education'] =='illiterate', 'Illiterate', data_nestor_b['education'])
data_nestor_b['education']=np.where(data_nestor_b['education'] =='unknown', 'Unknown', data_nestor_b['education'])
#Check the values of who purchased the deposit account
print(data_nestor_b['y'].value_counts())
##Check the average of all the numeric columns
pd.set_option('display.max_columns',100)
print(data_nestor_b.groupby('y').mean())
#Check the mean of all numeric columns grouped by education
print(data_nestor_b.groupby('education').mean())
"""
Plotting
"""
#Plot a histogram showing purchase by education category
import matplotlib.pyplot as plt
#  HELP CROSSTAB help(pd.crosstab)
pd.crosstab(data_nestor_b.education,data_nestor_b.y)
#Create a histogram
pd.crosstab(data_nestor_b.education,data_nestor_b.y).plot(kind='bar')
...
table=pd.crosstab(data_nestor_b.marital,data_nestor_b.y)
...
pd.crosstab(data_nestor_b.day_of_week,data_nestor_b.y).plot(kind='bar')
plt.title('Purchase Frequency for Day of Week')
plt.xlabel('Day of Week')
plt.ylabel('Frequency of Purchase')
###
print(data_nestor_b['day_of_week'].unique())
###
#Repeat for the month
pd.crosstab(data_nestor_b.month,data_nestor_b.y).plot(kind='bar')
plt.title('Purchase Frequency for Day of Week')
plt.xlabel('Month')
plt.ylabel('Frequency of Purchase')
###
#Plot a histogram of the age distribution
data_nestor_b.age.hist()
...
#  HELP DUMMIES help(pd.get_dummies)
cat_vars=['job','marital','education','default','housing','loan','contact','month','day_of_week','poutcome']
for var in cat_vars:
    cat_list='var'+'_'+var
    print(cat_list)
    cat_list = pd.get_dummies(data_nestor_b[var], prefix=var)
    data_nestor_b1=data_nestor_b.join(cat_list)
    data_nestor_b=data_nestor_b1
data_nestor_b.head(5)
 #######
#  2- Removee the original columns
cat_vars=['job','marital','education','default','housing','loan','contact','month','day_of_week','poutcome']
data_nestor_b_vars=data_nestor_b.columns.values.tolist()
to_keep=[i for i in data_nestor_b_vars if i not in cat_vars]
#
data_nestor_b_final=data_nestor_b[to_keep]
data_nestor_b_final.columns.values
####  
# 3- Prepare the data for the model build as X (inputs, predictor) and Y(output, predicted)
data_nestor_b_final_vars=data_nestor_b_final.columns.values.tolist()
Y=['y']
X=[i for i in data_nestor_b_final_vars if i not in Y ]
type(Y)
type(X)
...
rfe = RFE(model, 12)
rfe = rfe.fit(data_nestor_b_final[X],data_nestor_b_final[Y])
print(rfe.support_)
print(rfe.ranking_)

data_nestor_b_final[X].columns[rfe.support_].values
...
X=data_nestor_b_final[cols]
Y=data_nestor_b_final['y']
type(Y)
type(X)
"""
